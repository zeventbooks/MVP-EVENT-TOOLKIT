...
4. **Composable partials:** Register partial snippets (Header, Footer, Toasts, etc.) as reusable modules rendered through a single helper so they stay deduplicated and testable.
   - Because each route explicitly declares its partial list, the renderer only loads the HTML fragments and modules required for that page instead of the "include everything" pattern used today. This directly reduces the number of templates the Agent (Apps Script runtime) has to evaluate on every request, so `?page=admin&brand=root` and `?page=admin&brand=ABC` hydrate faster while still sharing the same layout shell.
