...
if (errors.length) {
  ...
  console.error('\nRemediation checklist (see "When the hierarchy falls out of line" in CODEBASE_CONSOLIDATION_STRATEGY.md):');
  console.error('  1. Record the deviation (token + file) in your working notes.');
  console.error('  2. Update CODEBASE_CONSOLIDATION_STRATEGY.md first, then mirror the fix in mind_mapping.md.');
  console.error('  3. Apply the change to the downstream doc and re-run npm run docs:lint.');
  process.exit(1);
}
