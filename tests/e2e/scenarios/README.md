...
> ⚠️ If you see "Deployment requires Google authentication" it means the repo-wide Playwright guard intercepted the Google login wall. Publish the Apps Script web app as "Anyone" or point `BASE_URL` at a deployment with anonymous access (see `TESTING.md`) before rerunning.
