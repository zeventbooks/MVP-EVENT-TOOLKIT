...
- **Out-of-line recovery:** When `npm run docs:lint` or a monitoring scenario fails, log the deviation (token + file), update `CODEBASE_CONSOLIDATION_STRATEGY.md` first, mirror the fix here, then touch downstream docs and re-run the validator + scenario before merging.

## 6. Systematic Drift Matrix
- Mirrors the table in the canon to keep every discipline honest:
  - **Docs & knowledge:** lint first, then cascade tokens downstream.
  - **Code & HTML:** guard with `npm run lint:apps-script-meta`, keep Apps Script URLs primary, leave legacy hosts commented.
  - **Automated tests:** sync `tests/config/environments.js`, rerun the affected Playwright/Jest suites, archive logs with scenario tokens.
  - **Process & DevOps:** reconcile `DEVOPS-WORKFLOW.md`, GitHub Actions, clasp scripts, and `scripts/monitor-health.js` automation outputs.
  - **Config & secrets:** update manifest + `appsscript.json`/`.clasp.json`, document rotations, attach diff snippets.
  - **Data & telemetry:** update `monitoring.targets*.json`, capture reports/history, pin dashboards.
  - **Agile roles:** refresh the Agile Team Analysis section before reassigning responsibilities anywhere else.
- Use the same detect → realign → prove loop for every row; no PR may merge without the proof artifacts listed in the strategy file.

## 7. Change Log
- *2025-01-15* – v2 adds monitoring blueprint alignment + consolidation rules sync.
- *2025-01-16* – v3 documents the cross-discipline drift matrix so remediation spans docs, code, tests, process, DevOps, configuration, data, and team roles.
